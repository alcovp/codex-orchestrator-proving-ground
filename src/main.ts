...
type GameState = "playing" | "exploding";
...
const gridHelper = new THREE.GridHelper(boardWidth, settings.columns, colors.grid, colors.grid);
...
const appleGeometry = new THREE.SphereGeometry(settings.tileSize * 0.35, 22, 22);
...
window.addEventListener("keydown", (event) => {
  ...
  if (event.key === "Enter" && gameState === "exploding") {
    event.preventDefault();
    resetRound();
  }
});
...
function animate(now: number): void {
  ...
  while (gameState === "playing" && tickAccumulatorMs >= settings.tickMs) {
